...
#define _cpu_overutilized(cap, max, cpu)	\
		((cap) * capacity_margin > (max) << SCHED_CAPACITY_SHIFT)
...
update_tg_cfs_runnable(struct cfs_rq *cfs_rq, struct sched_entity *se, struct cfs_rq *gcfs_rq)
{
	...
	delta_sum = runnable_load_avg - se->avg.runnable_load_avg;
...
static int find_energy_efficient_cpu(struct task_struct *p, int prev_cpu)
{
	 ...
	 * on !active CPUs.
	 */
	if (unlikely(!cpumask_and(&allowed, p->cpus_ptr, cpu_active_mask)))
	...
	for_each_cpu(i, &allowed) {
		...
		if (!_cpu_overutilized(p_util, cap[i], i)) {
